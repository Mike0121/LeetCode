...
class Solution:
    def maxAreaOfIsland(self, grid: List[List[int]]) -> int:
        ...
        def check_valid_position(row, col):
            if 0 <= row < num_rows and 0 <= col < num_cols and grid[row][col] == ISLAND:
                return True
            else:
                return False

        def calculate_island_area(row, col):
            if not check_valid_position(row, col):
                return 0
        ...
        for r in range(num_rows):
            for c in range(num_cols):
                if grid[r][c] == 1:
                    max_island_area = max(max_island_area, calculate_island_area(r, c))
